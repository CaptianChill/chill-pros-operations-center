...
export default async function handler(req, res) {
  try {
    const response = await fetch(`${SOURCE}/`, {
      headers: { 'user-agent': 'ChillProsCanonicalVisualProxy/3.0' },
      cache: 'no-store',
    });
    if (!response.ok) {
    ...
    // inject only scoped auth/billing/Chill Bro overlays after the visual shell.
    html = html
      .replace(/(src|href)="\/(?!\/)/g, `$1="${SOURCE}/`)
      .replace(/(src|href)='\/(?!\/)/g, `$1='${SOURCE}/`)
      .replace(/"\/_next\//g, `"${SOURCE}/_next/`)
      .replace(/'\/_next\//g, `'${SOURCE}/_next/`)
      .replace('</head>', `<meta name="chill-pros-visual-contract" content="${SOURCE_ID}"><link rel="stylesheet" href="/canonical-overlay.css?v=20260828-pinned1"></head>`)
      .replace('</body>', '<script src="/canonical-auth.js?v=20260828-pinned1"></script><script src="/canonical-top-level.js?v=20260828-pinned1"></script></body>');
